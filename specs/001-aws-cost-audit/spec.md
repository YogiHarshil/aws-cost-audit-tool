...
### Session 2026-05-13
- Q: How should S3 bucket sizes be obtained for the >100GB threshold check? → A: Use CloudWatch `BucketSizeBytes` metric (updated daily, efficient)
- Q: Where should AWS pricing data come from for savings calculations? → A: Query AWS Pricing API in real-time for accurate, current pricing
- Q: How to handle resources with missing CloudWatch metrics data? → A: Skip resource with warning in logs, don't flag as wasteful (prevents false positives)
- Q: What should happen if OpenAI API fails during report generation? → A: Continue with partial AI summaries, mark failed sections as "AI summary unavailable"
- Q: Should scanner support tag-based resource exclusions? → A: Yes, support optional tag exclusion filters via CLI/config (flexible, client-specific)
...
**Pricing Data:** All savings calculations use real-time pricing from AWS Pricing API (us-east-1 endpoint) to ensure accuracy. Pricing queries are cached per scan session to minimize API calls.

**Tag-Based Exclusions:** All scanners support optional tag-based exclusion filters. Resources matching any exclusion tag (e.g., `Environment=Production`) are skipped and not flagged as wasteful. Configured via `--exclude-tags` CLI option or `EXCLUDE_TAGS` environment variable.
...
- **Low utilization running instances (<5% CPU over 14 days):**
  ...
  - Severity: Medium
  - Note: If no CloudWatch data available, skip instance with warning (don't flag as wasteful)
...
- **Large buckets without tiering:**
  ...
  - Obtain bucket size via CloudWatch `BucketSizeBytes` metric (StandardStorage class, latest datapoint)
...
- **Data collected:** Bucket name, region, size (from CloudWatch), storage class distribution, lifecycle policy status, creation date
...
- If OpenAI API fails during generation (rate limits, timeout, service outage):
  - Continue report generation with partial results
  - Mark failed sections as "AI summary unavailable"
  - Log error details for operator review
  - Report remains deliverable with all findings intact
...
- `--exclude-tags TAGS`: Comma-separated tag filters to exclude resources (e.g., `Environment=Production,Critical=true`)
...
2. Parse tag exclusion filters if provided
3. Validate AWS credentials and OpenAI key (if AI enabled)
4. Discover enabled regions
5. Scan services in parallel (ThreadPoolExecutor), applying tag exclusions
6. Aggregate findings
7. Generate AI summaries (if enabled)
8. Render HTML template
9. Convert HTML to PDF via WeasyPrint
10. Save to `output/` directory
11. Print summary: "Report saved to {path}, Total savings: ${amount}"
...
- AWS API rate limits: Exponential backoff, continue with partial results
- OpenAI API failures: Continue with partial AI summaries, mark unavailable sections
- Service access denied: Log warning, skip that service, continue scan
- Region unavailable: Log warning, skip region, continue
- Missing CloudWatch metrics: Log warning, skip resource (don't flag as wasteful)
...
- Pricing API queries cached per scan session to minimize overhead
...
- Continue report generation if OpenAI API fails (mark failed sections)
...
│   └── pricing.py      # AWS Pricing API queries and calculations
├── tests/
│   ├── test_models.py
│   ├── test_scanners.py
│   ├── test_ai.py
...
- Implement utils/pricing.py (query AWS Pricing API for real-time rates)
